...
create table public.weekly_reports (
  ...
  -- star rating, date); the verbatim quote lives in report_quote_snapshots
  -- below (theme_kind 'danger') so it stays subject to the 30-day purge.
...
-- Evidence quotes backing a report's good/bad themes, or a danger-flag
-- alert — same purge semantics as meeting_quote_snapshots/
-- sop_evidence_quotes: content_purge_at is copied from the source review
-- at insert time, not reset, so a quote leaves the saved report on the
-- same schedule it would have left the reviews table. Danger-flag rows
-- (theme_kind 'danger') aren't tied to the sentiment taxonomy, so
-- category is null and flag is set instead; good/bad rows are the
-- reverse.
create table public.report_quote_snapshots (
  id                uuid primary key default uuid_generate_v4(),
  tenant_id         uuid not null,
  report_id         uuid references public.weekly_reports(id) on delete cascade not null,
  theme_kind        text not null check (theme_kind in ('good','bad','danger')),
  category          text check (category in ('food','service','atmosphere','value','wait_time','cleanliness')),
  flag              text check (flag in ('health_safety','legal','discrimination','physical_safety')),
  review_id         uuid references public.reviews(id) on delete set null,
  location_id       uuid references public.locations(id) on delete set null,
  location_name     text not null,
  quote_text        text,              -- nulled at content_purge_at
  ...
  reviewed_at       timestamptz,
  content_purge_at  timestamptz not null,
  constraint report_quote_snapshots_kind_shape check (
    (theme_kind in ('good','bad') and category is not null and flag is null) or
    (theme_kind = 'danger' and flag is not null and category is null)
  )
);
